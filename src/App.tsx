...
function App() {
    ...
    const connect = () => {
      ...
      socket.onmessage = (event) => {
        try {
          ...
          else if (data.type === "sync") { elementsRef.current = data.elements; setElements(data.elements); setRedoStack([]); setSelectedId(null); }
          ...
          else if (data.type === "remove") setElements((previous) => { const updated = previous.filter((item) => item.id !== data.id); elementsRef.current = updated; if (selectedIdRef.current === data.id) setSelectedId(null); return updated; });
          else if (data.type === "clear") { elementsRef.current = []; setElements([]); setRedoStack([]); setSelectedId(null); }
  ...
  const deleteSelected = () => {
    ...
    if (!id) return;
    elementsRef.current = elementsRef.current.filter((item) => item.id !== id);
    setElements(elementsRef.current);
    setRedoStack([]);
    setSelectedId(null);
    send({ type: "remove", room, id });
  };
  ...
  const startDrawing = (event: PointerEvent<HTMLCanvasElement>) => {
    ...
    if (tool === "select") {
      ...
      if (selected && handle) {
        dragRef.current = { start: point, original: { ...selected, points: selected.points.map((p) => ({ ...p })) }, mode: "resize", handle };
      } else {
        const hit = hitElement(point);
        setSelectedId(hit?.id || null);
        if (hit) dragRef.current = { start: point, original: { ...hit, points: hit.points.map((p) => ({ ...p })) }, mode: "move", handle: null };
      }
      return;
    }

    setIsDrawing(true); setStartPoint(point); setCurrentPoint(point);
  ...
  const draw = (event: PointerEvent<HTMLCanvasElement>) => {
    ...
    if (tool === "select") {
      ...
      if (drag.mode === "move") {
        ...
        const b = getBounds(drag.original);
        const h = drag.handle!;
        const anchorX = h.includes("e") ? b.minX : b.maxX;
        const anchorY = h.includes("s") ? b.minY : b.maxY;
        const newX = h.includes("e") ? Math.max(anchorX + 4, point.x) : Math.min(anchorX - 4, point.x);
        const newY = h.includes("s") ? Math.max(anchorY + 4, point.y) : Math.min(anchorY - 4, point.y);
        const oldW = Math.max(1, b.maxX - b.minX), oldH = Math.max(1, b.maxY - b.minY);
        const newW = Math.max(4, Math.abs(newX - anchorX)), newH = Math.max(4, Math.abs(newY - anchorY));
        const sx = newW / oldW, sy = newH / oldH;
        updatedElement = { ...drag.original, points: drag.original.points.map((p) => ({ x: anchorX + (p.x - anchorX) * sx, y: anchorY + (p.y - anchorY) * sy })) };
      }
      const updated = elementsRef.current.map((item) => item.id === updatedElement.id ? updatedElement : item);
      elementsRef.current = updated; setElements(updated); setRedoStack([]); send({ type: "draw", room, element: updatedElement });
      return;
    }
    ...
    if (tool === "pen" || tool === "eraser") {
      const id = activeElementId.current, current = elementsRef.current.find((item) => item.id === id);
      if (!id || !current) return;
      const updatedElement = { ...current, points: [...current.points, point] };
      const updated = elementsRef.current.map((item) => item.id === id ? updatedElement : item);
  ...
  const stopDrawing = (event?: PointerEvent<HTMLCanvasElement>) => {
    ...
    if (startPoint && currentPoint && tool !== "pen" && tool !== "eraser") {
      const element: Element = { id: crypto.randomUUID(), type: tool, points: [startPoint, currentPoint], color, size, user: nameRef.current };
      const updated = [...elementsRef.current, element]; elementsRef.current = updated; setElements(updated); setRedoStack([]); send({ type: "draw", room, element });
    }
  ...
  const undo = () => {
    const own = elementsRef.current.filter((item) => item.user === nameRef.current), last = own[own.length - 1];
    if (!last) return;
    const updated = elementsRef.current.filter((item) => item.id !== last.id); elementsRef.current = updated; setElements(updated); setRedoStack((previous) => [...previous, last]); if (selectedIdRef.current === last.id) setSelectedId(null); send({ type: "remove", room, id: last.id });
  };

  const redo = () => {
    const last = redoStack[redoStack.length - 1]; if (!last) return;
    const updated = [...elementsRef.current, last]; elementsRef.current = updated; setElements(updated); setRedoStack((previous) => previous.slice(0, -1)); send({ type: "draw", room, element: last });
  };

  const clearBoard = () => { if (!window.confirm(`Clear the entire '${room}' room for everyone?`)) return; elementsRef.current = []; setElements([]); setRedoStack([]); setSelectedId(null); send({ type: "clear", room }); };
  ...
  return <div className="app">
    <header className="topbar">
      ...
      <div className="top-actions">
        ...
        <input className="room-input" value={roomInput} onChange={(event) => setRoomInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") switchRoom(roomInput); }} placeholder="Room name" aria-label="Room name" autoComplete="off" />
    ...
    <div className="toolbar">
      ...
      <button className="action" onClick={deleteSelected} disabled={!selectedId}>Delete</button>
    ...
    <main className="workspace"><section className="canvas-wrapper">
      <div className="canvas-header"><span>Room: <strong>{room}</strong></span><span>{elements.length} objects · {selectedId ? "1 selected · " : ""}{Math.round(zoom * 100)}% zoom</span></div>
      <div className="canvas-scroll"><div className="canvas-stage" style={{ width: WIDTH * zoom, height: HEIGHT * zoom }}>
        <canvas ref={canvasRef} className="canvas" width={WIDTH} height={HEIGHT} style={{ width: WIDTH, height: HEIGHT, transform: `scale(${zoom})`, cursor: tool === "select" ? "default" : "crosshair" }} onPointerDown={startDrawing} onPointerMove={draw} onPointerUp={stopDrawing} onPointerCancel={stopDrawing} onPointerLeave={(event) => { if (!isDrawing && tool !== "select") sendCursor(event); }} />
