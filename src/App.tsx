...
function App() {
  ...
  const switchRoom = (value: string) => {
    ...
    if (next === room) {
      window.alert(`You are already in room: ${next}`);
      return;
    }
    elementsRef.current = [];
    setElements([]);
  ...
  const joinRoom = () => {
    // Native prompt is intentional: it guarantees room selection works even if a browser/UI layer prevents typing in the field.
    const entered = window.prompt("Enter the room name:", roomInput || room);
    if (entered === null) return;
    switchRoom(entered);
  };

  const joinFromInput = () => switchRoom(roomInput);
  ...
  const drawElement = (ctx: CanvasRenderingContext2D, element: Element) => {
    ...
    ctx.arc(first.x, first.y, Math.sqrt(dx * dx + dy * dy), 0, Math.PI * 2);
    ctx.stroke();
  };
    ...
    const connect = () => {
      ...
      socket.onmessage = (event) => {
        try {
          ...
          else if (data.type === "cursor" && data.id !== myIdRef.current) {
            setCursors((previous) => ({ ...previous, [data.id]: { x: data.x, y: data.y, name: data.name } }));
  ...
  const sendCursor = (event: PointerEvent<HTMLCanvasElement>) => {
    const point = getPoint(event);
    send({ type: "cursor", room, x: point.x, y: point.y, name: nameRef.current });
  };
  ...
  const undo = () => {
    ...
    send({ type: "remove", room, id: last.id });
  };
  ...
  const redo = () => {
    ...
    send({ type: "draw", room, element: last });
  };
    ...
    <div className="app">
      <header className="topbar">
        <div className="brand">
          ...
          <div><h1>Canvasly</h1><span>Real-time collaborative canvas</span></div>
        </div>
        ...
        <div className="top-actions">
          ...
          <input
            ...
            onKeyDown={(event) => { if (event.key === "Enter") joinFromInput(); }}
            placeholder="Room name"
            aria-label="Room name"
            autoComplete="off"
          />
          <button className="secondary" onClick={joinRoom}>Join Room</button>
          <button className="secondary" onClick={() => joinFromInput()}>Use Room</button>
      ...
      <div className="toolbar">
        <div className="tool-group">
          {tools.map((item) => <button key={item.id} className={`tool ${tool === item.id ? "active" : ""}`} onClick={() => setTool(item.id)}>{item.label}</button>)}
        </div>
      ...
      <main className="workspace">
        <section className="canvas-wrapper">
          <div className="canvas-header">
            ...
            <span>{elements.length} objects · {Math.round(zoom * 100)}% zoom</span>
          </div>
          <div className="canvas-scroll">
            <div className="canvas-stage" style={{ width: WIDTH * zoom, height: HEIGHT * zoom }}>
              <canvas
                ref={canvasRef}
                className="canvas"
                width={WIDTH}
                ...
                onPointerLeave={(event) => { if (!isDrawing) sendCursor(event); }}
              />
              <div className="cursor-layer">
                {Object.entries(cursors).map(([id, cursor]) => (
                  <div key={id} className="remote-cursor" style={{ left: cursor.x * zoom, top: cursor.y * zoom }}>
                    <div className="cursor-arrow">↖</div><span className="cursor-name">{cursor.name}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <footer>
        <span>Canvasly • Room <strong>{room}</strong></span>
        <span>Changes sync automatically and persist in the database.</span>
      </footer>
    </div>
  );
}
